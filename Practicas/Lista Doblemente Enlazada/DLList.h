...
public:
    ...
    public:
        ...
        //hacernos para atras
        //Una lista simple solo tienes caminos para adelante mientras que una doble tiene soble sentido
        iterator &operator--() {
            if(current)
...
public: //Valores públicos
    ...
    //Cuando se llame al constructor ya tiene que estar establecida una caberza y una cola
    //Es un constructor pues
    ...
    //No se muy bien que hacer el amperson en este caso
    Object &front() {
        if (empty())
    ...
    //Funciones de push
    //Aqui se hace push al frente de la lista
    ...
    //Se necesista un iterador y se manda la posicion como paramatro
    //Baciamente se usa cuando se da un numero en vez de un iterador y lo traduce
    ...
    //Y borra el dato en la posicion dada
    iterator erase(iterator itr) {
        ...
        //p avanza al siguiente nodo hasta volverse el nodo que el usuario quiere borrar
        while (p->next != itr.current)
        {
            p = p->next;
        }
        //El siguiente de p se vuelve el siguiente al nodo que el usuario quiere quitar
        //(osea que estan en la misma posicion)
